...
class ContextBandit:
    ...
    def select_action(self, state, action_space, device="cuda"):
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                rl_q = self.policy_value_net.get_q(batch, temperature=self.kT)["rl_q"]
                print(rl_q.shape)
        ...
        action = np.random.choice(len(action_probs.detach().cpu().numpy()), p=action_probs.detach().cpu().numpy());
    ...
    def update(self, memory_l, gamma, steps, batch_size=8, device="cuda"):
        self.policy_value_net.to(device)
        ave_loss = 0;
        loss_fn = WeightedSumLoss(keys=("barrier", "freq"),weights=(1.0, self.kT**2), loss_fns=("mse_loss", "mse_loss"))
        ...
        for m in range(steps[0]):
            ...
            delta_e = torch.tensor([val- memory.E_min[i] for val in  enumerate(memory.E_next)], dtype=torch.float)

            total_dataset_list = []
            for i, state in enumerate(states):
                dataset = convert(states=[state],event_list=[taken_actions])
            ...
            for i, data in enumerate(total_dataset_list):
                data.q1 = freq[i]
                data.q0 = rewards[i]
                data.delta_e = delta_e[i]
                data.freq = freq[i]
                data.barrier = -rewards[i]
            ...
            for epoch in range(steps[1]):
                for batch in q_dataloader:
                    ...
                    print(batch["q0"])
                    print(batch["q1"])
                    output = self.policy_value_net(batch, temperature=self.kT, alpha=1.0, beta=0.0)
                    # output = self.policy_value_net.get_q(batch, temperature=self.kT, alpha=1.0, beta=0.0)
                    print(output)

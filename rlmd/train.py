...
class Context_Bandit:
    ...
    def select_action(self, state, action_space):
        Q = self.policy_value_net(state,action_space);
        action_probs = nn.Softmax()(Q/self.kT);
        action = np.random.choice(len(action_probs.detach().numpy()), p=action_probs.detach().numpy());
        
        return action, action_probs, Q;

    def update(self, memory_l, gamma):
        for m in range(len(memory_l)):
            memory = memory_l[m]
            states = memory.states
            aspace = memory.act_space;
            actions = memory.actions;
            ...
            for i in range(len(states)):
                Q = self.policy_value_net(states[i],[aspace[i][actions[i]]]);
                if(i==0 and m==0):
                    loss = (rewards[i]-Q)**2;
                else:
                    loss += (rewards[i]-Q)**2;
        loss = loss/len(states)/len(memory_l);
        self.optimizer.zero_grad();
        loss.backward();
        self.optimizer.step();
